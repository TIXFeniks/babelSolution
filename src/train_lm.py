...
def train_model(model_name, config):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if config.get('inp_embeddings_path'):
            ...
            sess.run(tf.assign(lm.emb_inp.trainable_weights[0], tf.constant(embeddings)))
        ...
        while should_start_next_epoch:
            # TODO(rykachevskiy) remove double src
            batches = batch_generator_over_dataset(src_train, src_train, batch_size, batches_per_epoch=None)
            with tqdm(batches) as t:
                for batch_src, batch_src in t:
