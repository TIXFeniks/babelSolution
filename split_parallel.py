...
with open('/input/parallel_corpus.txt', encoding = 'utf-8') as f:
     lines = [x[:-1] for x in f.readlines()]
...
with open('/nmt/data/parallel_train1.txt', 'wb') as f:
    f.write('\n'.join([x.split('\t')[0] for x in lines_train]).encode('utf-8'))
with open('/nmt/data/parallel_train2.txt', 'wb') as f:
    f.write(''.join([x.split('\t')[1] for x in lines_train]).encode('utf-8'))
with open('/nmt/data/parallel_val1.txt', 'wb') as f:
    f.write('\n'.join([x.split('\t')[0] for x in lines_val]).encode('utf-8'))
with open('/nmt/data/parallel_val2.txt', 'wb') as f:
